...
if ( ! class_exists( 'Templator_Import_Core' ) ) :
	...
	class Templator_Import_Core {
		...
		private function get_saved_image( $attachment ) {
			if ( apply_filters( 'templator_image_importer_skip_image', false, $attachment ) ) {

				Templator_Import_Core::log( 'Download (✕) Replace (✕) - ' . $attachment['url'] );
			...
			$post_id = $wpdb->get_var(
				$wpdb->prepare(
					'SELECT `post_id` FROM `' . $wpdb->postmeta . '`
						WHERE `meta_key` = \'_templator_image_hash\'
							AND `meta_value` = %s
					;',
		...
		public function import( $attachment ) {
			...
			if ( empty( $file_content ) ) {
				...
				Templator_Import_Core::log( 'Download (✕) Replace (✕) - ' . $attachment['url'] );
				Templator_Import_Core::log( 'Error: Failed wp_remote_retrieve_body().' );
			...
			update_post_meta( $post_id, '_templator_image_hash', $this->get_hash_image( $attachment['url'] ) );
	 ...
	 */
	Templator_Import_Core::get_instance();
